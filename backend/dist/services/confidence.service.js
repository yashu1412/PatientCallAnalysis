...
exports.computeConfidenceScore = void 0;

const computeConfidenceScore = (input) => {
    const base = input.transcriptConfidence ?? 0.75;
    let penalty = 0;
    const flags = input.qualityFlags;
    if (flags.includes("LOW_CONFIDENCE_TRANSCRIPT"))
        penalty += 0.15;
    if (flags.includes("POSSIBLE_CODE_SWITCHING"))
        penalty += 0.10;
    if (flags.includes("HIGH_INAUDIBLE_RATE"))
        penalty += 0.10;
    if (flags.includes("TRANSCRIPT_TOO_SHORT"))
        penalty += 0.10;
    if (flags.includes("REPEATED_BROKEN_WORDS"))
        penalty += 0.05;
    if (flags.includes("MISSING_CRITICAL_CONTEXT"))
        penalty += 0.10;
